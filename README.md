...
2. 刷内核会影响恢复出厂设置吗？  
影响，由于OPPO的不完全开源，GKI内核恢复出厂设置很容易砖，需要fastboot刷一遍全量包才能开机，天玑甚至可能需要售后刷机才能恢复。非必要不要恢复出厂设置，若实在需要，请刷回原厂boot.img.  
...
首先，答案是不能，防格机针对已知的格机手段尽可能实现了最大保护，但安全性与便捷性永远也不可能平衡，为了不对用户的正常操作产生干扰，防格机仍然存在少数绕过方式，请自行甄别运行的程序和脚本的可信度。其次，防格机这个叫法并不准确，bbg是基带守护，保护关键分区避免主板返厂，被格机后仍然可能黑砖、用户数据丢失，只是更容易救砖。  
8. 如何验证防格机是否生效？  
